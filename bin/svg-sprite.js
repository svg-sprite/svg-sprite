...
function addOption(name, option) {
  ...
  if ('description' in option) {
    ...
    if ('default' in option) {
      ...
      const def = template
        ? path.resolve(path.dirname(__dirname), option.default)
        : option.default;
...
if (argv.config) {
  try {
    ...
    console.error(
      '[ERROR] Skipping --config file due to errors ("%s")',
      error.message.trim()
    );
  }
}
...
config.shape.spacing.padding = config.shape.spacing.padding.length
  ? config.shape.spacing.padding
      .split(',')
      .map(dim => Number.parseFloat(dim || 0))
  : [];

if (
  config.svg.rootAttributes &&
  typeof config.svg.rootAttributes === 'string'
) {
  try {
    const JSONAttributesContent = fs.readFileSync(
      path.resolve(config.svg.rootAttributes)
    );
    config.svg.rootAttributes = JSON.parse(JSONAttributesContent);
  } catch (error) {
    console.error(
      '[ERROR] Skipping --svg-rootattrs file due to errors ("%s")',
      error.message.trim()
    );
    config.svg.rootAttributes = {};
  }
...
if (typeof config.shape.transform === 'string') {
  ...
  (transform.length
    ? transform.split(',').map(trans => String(trans).trim())
    : []
  )
    // TODO
      ...
      if (transform.length) {
        if (`shape-transform-${transform}` in argv) {
          try {
            ...
            const transformConfigJSON = fs.readFileSync(
              path.resolve(transformConfigFile),
              'utf8'
            );
            const transformConfig = transformConfigJSON.trim()
              ? JSON.parse(transformConfigJSON)
              : {};
...
if ('variables' in config) {
  ...
  if (variables && fs.existsSync(variables)) {
    try {
      ...
      console.error(
        '[ERROR] Skipping --variables file due to errors ("%s")',
        error.message.trim()
      );
    }
  }
...
for (let file of files) {
  ...
  if (stat.isSymbolicLink()) {
    ...
    basename =
      basepos >= 0 ? basename.substr(basepos + 2) : path.basename(file);
  }
