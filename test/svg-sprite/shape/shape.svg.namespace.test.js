...
describe('testing setNamespace()', () => {
    ...
    describe('if namespaceIds', () => {
        it('should call multiple xpath.select and set attributes', () => {
            ...
            expect(THIRD_ELEMENTS[1].ownerElement.setAttribute).toHaveBeenCalledWith(
                'href',
                `#${TEST_NAMESPACE}${TEST_ATTR_VALUE}`
            );
        ...
        it('should set aria-labelledby', () => {
            ...
            expect(shape.dom.documentElement.setAttribute).toHaveBeenCalledWith(
                'aria-labelledby',
                `${TEST_NAMESPACE}${TEST_ATTR_VALUE} test`
            );
        });
    });
    ...
    describe('with namespaceClassnames', () => {
        it('should call xpath.select with //*[@class]', () => {
            ...
            expect(TEST_ELEMENTS[0].setAttribute).toHaveBeenCalledWith(
                'class',
                `${TEST_NAMESPACE}1 ${TEST_NAMESPACE}2 ${TEST_NAMESPACE}3 ${TEST_NAMESPACE}4 ${TEST_NAMESPACE}5 ${TEST_NAMESPACE}6`
            );
            expect(shape._namespaced).toBe(true);
        });
    });
