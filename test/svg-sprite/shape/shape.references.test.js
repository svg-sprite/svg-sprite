...
describe('testing _replaceIdAndClassnameReferences()', () => {
  it('should replace ids if subs ids passed', () => {
    ...
    expect(
      shape._replaceIdAndClassnameReferences(
        TEST_STRING,
        TEST_SUBS_IDS,
        {},
        false
      )
    ).toBe('url(#NEW ID 1) url(id2)');
  });
  ...
  it('should not change string if subs ids is null', () => {
    ...
    expect(
      shape._replaceIdAndClassnameReferences(TEST_STRING, null, {}, false)
    ).toBe(TEST_STRING);
  });
  ...
  it('should return value of _replaceIdAndClassnameReferencesInCssSelectors() if selectors passed', () => {
    ...
    jest
      .spyOn(shape, '_replaceIdAndClassnameReferencesInCssSelectors')
      .mockReturnValueOnce(TEST_RESULT);
    ...
    expect(
      shape._replaceIdAndClassnameReferencesInCssSelectors
    ).toHaveBeenCalledWith(TEST_STR, '', TEST_SUBS_IDS, TEST_SUBST_CLASSNAMES);
  });
});
...
describe('testing _replaceIdAndClassnameReferencesInCssSelectors()', () => {
  ...
  it('should follow selectors', () => {
    ...
    expect(expected).toBe(
      `div.cls1:has(div),#NO${TEST_STRING}.NOCLS,.y,.yy${TEST_STRING}a#b[href="test"]str`
    );
  });
});
