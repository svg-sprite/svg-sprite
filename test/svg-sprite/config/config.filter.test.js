...
describe('testing filter', () => {
  ...
  it.each(modes)(
    'should filter plainObjects and set %p mode if mode is not specified',
    mode => {
      expect.hasAssertions();
      expect(
        config.filter({
  ...
  it.each(modes)(
    'should filter plainObjects and set mode %p if mode is specified',
    mode => {
      expect.hasAssertions();
      expect(
        config.filter({
  ...
  it.each(modes)(
    'should ignore if passed value is not object and not true',
    mode => {
      expect.hasAssertions();
      expect(
        config.filter({
