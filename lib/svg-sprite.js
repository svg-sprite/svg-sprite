...
SVGSprite.prototype._processFiles = function(callback) {
	...
	async.parallel(tasks, function (error, results) {
		...
		svgIDs.forEach(function(svgID, index, array){
			this._addToSprite(svgID, results[svgID], svgPseudos[svgID], (index === array.length - 1));
		}, that);
 ...
 * @param {Boolean} isLastSVG		Helper for Mustache to determine if last SVG in sprite
 */
SVGSprite.prototype._addToSprite = function(svgID, svgInfo, svgPseudo, isLastSVG) {
	...
	var positionX				= this._options.padding ? '-' + this._options.padding + 'px' : '0',
		positionY				= (-this.height - this._options.padding) + 'px';
