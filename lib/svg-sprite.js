 ...
 *
 * @param {string} name The string to trim the start of.
 * @param {string[]} [charsToTrim=[]] The characters to trim from the start of the string.
 * @returns {string} The trimmed string.
 */
function trimStart(name, charsToTrim = []) {
  const nameArray = Array.from(name);
  const firstNonTrimCharIndex = nameArray.findIndex(
    (char) => !charsToTrim.includes(char)
  );
  return nameArray.slice(firstNonTrimCharIndex).join("");
}
