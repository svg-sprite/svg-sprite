...
const {
  ...
  isPlainObject,
  zipObject
} = require('./svg-sprite/utils/index.js');
...
SVGSpriter.prototype.add = function (file = '', name = '', svg = '') {
  ...
  if (!this._isVinylFile(file)) {
    ...
    if (name && path.isAbsolute(name)) {
      errorMessage = format(
        'SVGSpriter.add: "%s" is not a valid relative file name',
        name
      );
    } else {
      ...
      if (arguments.length < 3) {
        ...
        errorMessage = format(
          'SVGSpriter.add: "%s" is not a valid absolute file name',
          file
        );
      } else if (!name.length) {
        errorMessage = format(
 ...
 *
 * This check is necessary as singletons don't work cross-module and
 * `instanceof` fails in these situations.
 *
  ...
  return (
    ...
    (file instanceof File ||
      (file.constructor.name === 'File' &&
        ['path', 'contents', 'relative'].filter(function (property) {
...
SVGSpriter.prototype._transformShape = function (shape, cb) {
  const createTransformTask = transform => {
    ...
    if (transform[0] in this._shapeTransformers && isObject(transform[1])) {
      return callback => {
        this._shapeTransformers[transform[0]](
          shape,
          transform[1],
          this,
          callback
...
SVGSpriter.prototype._compile = function () {
  ...
  if (Object.keys(config).length === 0) {
    ...
    while (
      !this._namespacePow.length ||
      26 ** this._namespacePow.length < masterShapes.length
    ) {
    ...
    this._layout(config, (error, files, data) => {
      ...
      if (this.config.shape.dest) {
        files.shapes = this._getShapeFiles(this.config.shape.dest);
        this.verbose(
          'Returning %d intermediate SVG files',
          files.shapes.length
        );
      }
...
SVGSpriter.prototype._logStats = function (files) {
  ...
  this.info(
    ...
    Object.entries(extensions)
      .map(ext => `${ext[1]} x ${ext[0].substr(1)}`)
      .join(', ')
  );
