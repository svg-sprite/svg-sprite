...
SVGSprite.prototype._readSVGFiles = function(inputDir) {
	...
	if(this._options.recursive){
		newFiles = wrench.readdirSyncRecursive(inputDir);
		var cb = function(file){
			return path.extname(file) === '.svg'; 
		};
		var newFiles = newFiles.filter(cb);
		newFiles.forEach(function(file){
			files.push(inputDir+'/'+file);
		});
	}else{
...
SVGSprite.prototype._addToSprite = function(svgID, svgInfo, svgPseudo, isLastSVG) {
	...
	var rootAttr				= {y: this.data.sheight};
	...
	this.data.svg.push({
		...
		positionX				: positionX,
		positionY				: positionY,
		position				: positionX + ' ' + positionY + (this.data.sheight ? 'px' : ''),
		dimensions				: {
