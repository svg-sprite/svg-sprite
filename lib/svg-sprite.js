...
SVGSprite.prototype._reset = function() {
	...
	this.height						= 0;
	this.sprite						= [];
	this.data						= {common: false, prefix: null, sprite: null, dims: false, svg: []};
...
SVGSprite.prototype._processFiles = function(callback) {
	...
	async.parallel(tasks, function (error, results) {
		...
		svgIDs.forEach(function(svgID){
			this._addToSprite(svgID, results[svgID], svgPseudos[svgID]);
		}, that);
 ...
 */
SVGSprite.prototype._addToSprite = function(svgID, svgInfo, svgPseudo) {
	...
	this.data.dims				= this._options.dims;
