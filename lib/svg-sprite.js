...
var CONFIG					= require('./svg-sprite/config'),
	LOGGER					= require('./svg-sprite/logger'),
	QUEUE					= require('./svg-sprite/queue'),
	SHAPE					= require('./svg-sprite/shape'),
 ...
 */
SVGSpriter.prototype.log = function(message, level) {
 ...
 * @return {SVGSpriter}					Self reference
 */
SVGSpriter.prototype.error = function(error, level) {
	throw new Error(this.logger._logMessageLevel.apply(this.logger, arguments).message);
	return this;
}
...
SVGSpriter.prototype._transformShape = function(shape, cb) {
	...
	for (var t = 0, task; t < this.config.transform.length; ++t) {
		task					= (function(transform){
			...
			} else if ((transform[0] in that._transformers) && _.isObject(transform[1])) {
				return function() {
					that._transformers[transform[0]](shape, transform[1], that, arguments[arguments.length - 1]);
				};
...
SVGSpriter.prototype.compile = function() {
	...
	config				= _.isPlainObject(args[0]) ? args.shift() : _.clone(this.config.mode, true),
	cb					= _.isFunction(args[0]) ? args.shift() : function(){};
...
SVGSpriter.prototype._compile = function() {
	...
	if (!this._queue.active && this._compileQueue.length) {
		...
		while (!this._namespacePow.length || (Math.pow(26, this._namespacePow.length) < this._shapes.length)) {
			this._namespacePow.unshift(Math.pow(26, this._namespacePow.length));
			_.invoke(this._shapes, 'resetNamespace');
		}
		
		// Sort shapes by ID
		this._shapes		= this._shapes.sort(function(shape1, shape2){
			return (shape1.id == shape2.id) ? 0 : ((shape1.id > shape2.id) ? 1 : -1);
		});
		
		// Set the shape namespaces
		this._shapes.map(function(shape, index){
...
SVGSpriter.prototype._layout = function(config, cb) {
	...
	for (var mode in config) {
		this.log('Initializing %s mode layout process', mode, this.logger.LEVEL_DEBUG);
	...
	async.parallelLimit(tasks, os.cpus().length * 2, function(error, data){
		cb(error, files, _.object(_.pluck(data, 'mode'), data));
	});
}
 ...
 * @return {SVGSpriter}					Self reference
 */
SVGSpriter.prototype._reset = function() {
	return this;
}
