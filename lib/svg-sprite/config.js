...
module.exports = class SVGSpriterConfig {
  ...
  _setupLogger(config) {
    ...
    if ('log' in config) {
      if (isWinstonLogger(config.log)) {
        ...
        isString(config.log) &&
        ['info', 'verbose', 'debug'].includes(config.log)
      ) {
        log = config.log;
      } else if (config.log) {
        log = 'info';
  ...
  _getMetaData() {
    ...
    const metaFile = isString(this.shape.meta)
      ? path.resolve(this.shape.meta)
      : null;
    let meta = metaFile;
  ...
  _getAlignmentData() {
    ...
    const alignFile = isString(this.shape.align)
      ? path.resolve(this.shape.align)
      : null;
    let align = alignFile;
    ...
    for (const [key, value] of Object.entries(align)) {
      ...
      for (const [tmpl, tmplValue] of Object.entries(value)) {
        const template = tmpl.length
          ? tmpl.includes('%s')
            ? tmpl
            : `%s${tmpl}`
          : '%s';
        const file = path.join(path.dirname(key), path.basename(key, '.svg'));
        alignmentData[file][template] = Math.max(
          0,
          Math.min(1, Number.parseFloat(tmplValue))
        );
      }
    }

    this.log.debug(
      'Processed alignment data file "%s"',
      path.basename(alignFile)
    );
  ...
  _prepareShape() {
    ...
    this.shape.dest =
      'dest' in this.shape ? String(this.shape.dest).trim() : '';
    this.shape.dest = this.shape.dest.length
      ? path.resolve(this.dest, this.shape.dest)
      : null;
  ...
  _setupSpacing() {
    this.shape.spacing =
      'spacing' in this.shape ? this.shape.spacing || {} : {};
   ...
   * @private
   *
   * @returns {Array[]} Array of transform configurations
   */
   ...
   *
   * @returns {Array | undefined}     Array of svg transform functions
   */
