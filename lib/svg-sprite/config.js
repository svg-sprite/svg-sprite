...
const Logger = require('winston/lib/winston/logger.js');
...
function SVGSpriterConfig(config) {
    ...
    if ('log' in config) {
        if (config.log instanceof Logger || (isObject(config.log) && config.log.level !== undefined && isObject(config.log.transports) && isFunction(config.log.log))) {
            this.log = config.log;
        } else {
            this.log = isString(config.log) && ['info', 'verbose', 'debug'].includes(config.log) ? config.log : (config.log ? 'info' : '');
        }
    }
...
SVGSpriterConfig.prototype.filter = function(config = {}) {
    ...
    for (const m in config) {
        const modeConfig = isPlainObject(config[m]) ? config[m] : (config[m] === true ? {} : null);
 ...
 *
 * @param {object} config                 Configuration
 * @returns {SVGSpriterConfig}            SVGSpriter configuration
 */
