...
function SVGShape(file, spriter) {
    ...
    const relativePath = this.source.base ? path.relative(this.source.base, this.source.path) : this.name;
    this.config = merge(merge({}, defaultConfig), this.spriter.config.shape || {});
    // todo: get rid of setting empty object if this.spriter.config.shape is not provided (https://github.com/svg-sprite/svg-sprite/pull/653#discussion_r841069781)
    ...
    this.id = this.config.id.generator(relativePath, this.source);
