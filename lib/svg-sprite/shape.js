...
const createIdGenerator = template => {
  ...
  const generator = function (name) {
    const pathname = this.separator
      ? name.split(path.sep).join(this.separator)
      : name;
    return format(
      template || '%s',
      path.basename(pathname.replace(/\s+/g, this.whitespace), '.svg')
    );
  };
...
const defaultConfig = {
  ...
  spacing: {
     ...
     * Box sizing strategy
     *
     * Might be 'content' (padding is added outside of the shape), 'padding'
     * (shape plus padding will make for the given maximum size) or 'contain'
     * (like 'padding', but size will be fixed instead of maximum)
     *
     * @type {string}
     */
    box: 'content'
...
function SVGShape(file, spriter) {
  ...
  this.name = this.source.path.substr(
    this.source.base.length + path.sep.length
  );
  ...
  this.config = merge(
    merge({}, defaultConfig),
    this.spriter.config.shape || {}
  );

  if (!isFunction(this.config.id.generator)) {
    this.config.id.generator = createIdGenerator(
      isString(this.config.id.generator)
        ? this.config.id.generator +
            (this.config.id.generator.includes('%s') ? '' : '%s')
        : '%s'
    );
  }
  ...
  this.meta =
    this.id in this.config.meta
      ? this.config.meta[this.id]
      : relative in this.config.meta
      ? this.config.meta[relative]
      : {};
  this.align = Object.entries({
    ...this.config.align['*'],
    ...(this.id in this.config.align
      ? this.config.align[this.id]
      : relative in this.config.align
      ? this.config.align[relative]
      : {})
  });
 ...
 */
SVGShape.prototype._stripInlineNamespaceDeclarations = function (
  element,
  nsMap
) {
  ...
  if (nsMap[''] === this.DEFAULT_SVG_NAMESPACE) {
    ...
    if (
      ...
      defaultNamespace.value === this.DEFAULT_SVG_NAMESPACE
    ) {
  ...
  if (!('xlink' in nsMap) || nsMap.xlink === this.XLINK_NAMESPACE) {
    ...
    if (
      ...
      xlinkNamespace.value === this.XLINK_NAMESPACE
    ) {
 ...
 *
 * @param   {boolean}              inline    Prepare for inline usage (strip
 *   redundant XML namespaces)
 * @param   {Function | undefined} transform Final transformer before
 *   serialization (operating on a clone)
 * @returns {string}                         SVG
 */
SVGShape.prototype.getSVG = function (inline, transform) {
  ...
  if (inline || this.master) {
    return new XMLSerializer().serializeToString(
      this._stripInlineNamespaceDeclarations(svg)
    );
  }
...
SVGShape.prototype._initSVG = function () {
  ...
  const validSVGRegExp =
    /<svg(?:\s+[a-z\d-:]+=(["']).*?\1)*\s*(?:(\/)|(>[^]*<\/svg))>/i;
  ...
  this.dom = new DOMParser({
    ...
    errorHandler(level, message) {
      throw new ArgumentError(
        format('Invalid SVG file (%s)', message.split('\n').join(' '))
      );
    }
...
SVGShape.prototype.complement = function (cb) {
  async.waterfall(
    ...
    error => {
      ...
      this.svg.ready = new XMLSerializer().serializeToString(
...
SVGShape.prototype._setDimensions = function (cb) {
  ...
  const horizontalPadding =
    ...
    Math.max(
      0,
      this.config.spacing.padding.right + this.config.spacing.padding.left
    );
  ...
  const verticalPadding =
    ...
    Math.max(
      0,
      this.config.spacing.padding.top + this.config.spacing.padding.bottom
    );
  const height = this.height + verticalPadding;

  // Does the shape need to be scaled?
  if (
    width > this.config.dimension.maxWidth ||
    height > this.config.dimension.maxHeight ||
    (forceScale &&
      width < this.config.dimension.maxWidth &&
      height < this.config.dimension.maxHeight)
  ) {
  ...
  if (forceScale) {
    const diffWidth =
      this.config.dimension.maxWidth - this.width - horizontalPadding;
    const diffHeight =
      this.config.dimension.maxHeight - this.height - verticalPadding;
...
SVGShape.prototype._addPadding = function (cb) {
  ...
  if (padding.top || padding.right || padding.bottom || padding.left) {
    ...
    this.setDimensions(
      this.width +
        this.config.spacing.padding.right +
        this.config.spacing.padding.left,
      this.height +
        this.config.spacing.padding.top +
        this.config.spacing.padding.bottom
    );
  }
...
SVGShape.prototype._addMetadata = function (cb) {
  ...
  if (
    'description' in this.meta &&
    isString(this.meta.description) &&
    this.meta.description.length
  ) {
    if (!this.description) {
      this.description = this.dom.documentElement.insertBefore(
        this.dom.createElementNS(this.DEFAULT_SVG_NAMESPACE, 'desc'),
  ...
  if (
    'title' in this.meta &&
    isString(this.meta.title) &&
    this.meta.title.length
  ) {
    if (!this.title) {
      this.title = this.dom.documentElement.insertBefore(
        this.dom.createElementNS(this.DEFAULT_SVG_NAMESPACE, 'title'),
        this.dom.documentElement.firstChild
      );
    }
...
SVGShape.prototype.setNamespace = function (ns) {
  ...
  const namespaceClassnames = Boolean(
    this.spriter.config.svg.namespaceClassnames
  );
  ...
  const select = xpath.useNamespaces({
    svg: this.DEFAULT_SVG_NAMESPACE,
    xlink: this.XLINK_NAMESPACE
  });
  let substIds = null;
  ...
  if (namespaceIds) {
    ...
    for (const xlink of select('//@xlink:href', this.dom)) {
      ...
      if (!xlinkValue.startsWith('data:') && xlinkValue in substIds) {
        xlink.ownerElement.setAttribute(
          'xlink:href',
          `#${substIds[xlinkValue]}`
        );
      }
    ...
    for (const refProperty of svgReferenceProperties) {
      for (const ref of select(`//@${refProperty}`, this.dom)) {
        ref.ownerElement.setAttribute(
          ref.localName,
          this._replaceIdAndClassnameReferences(
            ref.nodeValue,
            substIds,
            substClassnames,
            false
 ...
 *
 * @returns {string}                  String with replaced ID and class name
 *   references
 */
SVGShape.prototype._replaceIdAndClassnameReferences = function (
  str,
  substIds,
  substClassnames,
  selectors
  ...
  return selectors
    ? this._replaceIdAndClassnameReferencesInCssSelectors(
        str,
        cssom.parse(str).cssRules,
        substIds,
        substClassnames
      )
    : str;
 ...
 */
SVGShape.prototype._replaceIdAndClassnameReferencesInCssSelectors = function (
  str,
  rules,
  substIds,
  substClassnames
  ...
  for (const rule of rules) {
    ...
    if (selText === undefined) {
      ...
      if (rule.keyText) {
        ...
        css +=
          str.substring(rule.__starts, rule.cssRules[0].__starts) +
          this._replaceIdAndClassnameReferencesInCssSelectors(
            str,
            rule.cssRules,
            substIds,
          ...
          str.substring(
            rule.cssRules[rule.cssRules.length - 1].__ends,
            rule.__ends
          );
      }
      ...
      const idOrClassSubstitution = sel => {
        ...
        if (
          'classNames' in sel.rule &&
          substClassnames !== null &&
          Array.isArray(sel.rule.classNames)
        ) {
      ...
      if (classnames.size) {
        ...
        for (const classname of sortedClassnames) {
          selText = selText
            .split(`.${classname}`)
            .join(`.${substClassnames[`.${classname}`]}`);
        }
      }
      ...
      css +=
        selText +
        str.substring(rule.__starts + origSelText.length, rule.__ends);
    }
  }
...
SVGShape.prototype.distribute = function () {
  ...
  for (const alignment of alignments) {
    ...
    copy.id =
      copy.base + (this.state ? this.config.id.pseudo + this.state : '');
