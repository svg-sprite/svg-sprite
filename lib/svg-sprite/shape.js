...
SVGShape.prototype._replaceIdAndClassnameReferencesInCssSelectors = function(str, rules, substIds, substClassnames) {
  ...
  for (const rule of rules) {
    ...
    if (selText === undefined) {
      ...
      if (rule.keyText) {
        css += rule.keyText + str.substring(rule.__starts, rule.__ends);

        // Else: Recursively process rule content
      } else if (Array.isArray(rule.cssRules)) {
        const firstRule = rule.cssRules[0];
        css += str.substring(rule.__starts, firstRule.__starts - (firstRule?.keyText?.length ?? 0)) + this._replaceIdAndClassnameReferencesInCssSelectors(str, rule.cssRules, substIds, substClassnames) + str.substring(rule.cssRules.at(-1).__ends, rule.__ends);
      }
