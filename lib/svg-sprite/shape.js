	...
	rules.forEach(function(rule) {
		...
		if (_.isUndefined(selText)) {
			if (_.isArray(rule.cssRules)) {
				css				+= str.substring(rule.__starts, rule.cssRules[0].__starts) + this._replaceIdReferencesInCssSelectors(str, rule.cssRules, subst) + str.substring(rule.cssRules[rule.cssRules.length - 1].__ends, rule.__ends);
			}
		} else {
