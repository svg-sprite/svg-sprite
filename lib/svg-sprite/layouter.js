...
module.exports = class SVGSpriteLayouter {
  ...
  constructor(spriter, config) {
    ...
    for (const [index, shape] of this._spriter._shapes.entries()) {
      ...
      this._commonData.shapes.push({
        ...
        fileSize: this.config.example
          ? pretty(shape.source.contents.length)
          : null
      });
    }
  ...
  layout(files, key, mode, cb) {
    ...
    const config = merge(
      merge(merge({}, defaultConfig[mode]), { svg: this._spriter.config.svg }),
      this.config[key] || {}
    );
    const data = merge(
      merge(merge({}, this._commonData), this._spriter.config.variables),
      config.variables
    );
