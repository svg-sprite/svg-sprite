...
module.exports = function(shape, config, spriter, cb) {
	...
	try {
		svgoInstance.optimize(svg, function(result) {
			...
			spriter.log('Optimized "%s" with SVGO (saved %s / %s%%)', shape.name, spriter.logger.bytes(svgLength - optimizedSVGLength), Math.round(100 * (svgLength - optimizedSVGLength) / svgLength), spriter.logger.LEVEL_DETAIL);
			cb(null);
		});
	} catch (error) {
		spriter.error('Optimizing "%s" with SVGO failed with "%s"', shape.name, error, spriter.logger.LEVEL_INFO);
		cb(error);
	}
}