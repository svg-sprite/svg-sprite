...
module.exports = function(shape, config, spriter, cb) {
	...
	try {
		svgoInstance.optimize(svg, function(result) {
			...
			spriter.log('Optimized "%s" with SVGO (saved %s / %s%%)', shape.name, pretty(svgLength - optimizedSVGLength), Math.round(100 * (svgLength - optimizedSVGLength) / svgLength), spriter.logger.LEVEL_NOTICE);
			cb(null);
		});
	} catch (error) {
		spriter.error('Optimizing "%s" with SVGO failed with error "%s"', shape.name, error, spriter.logger.LEVEL_ERROR);
		cb(error);
	}
}