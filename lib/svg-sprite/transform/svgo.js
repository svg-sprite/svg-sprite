 ...
 */
function svgoTransform(shape, config, spriter, cb) {
    ...
    try {
        ...
        for (let t = 0; t < spriter.config.log.transports.length; t++) {
            if (spriter.config.log.transports[t].level === 'debug') {
                ...
                const diff = optSVGLength - svgLength;
                const percentage = Math.round(100 * (diff / svgLength));
                spriter.debug('Optimized "%s" with SVGO (%s / %s%%)', shape.name, prettyBytes(diff), percentage);
            }
        }
