    ...
    Object.defineProperty(dataShape, 'svg', {
      get() {
        return this._svg || shape.getSVG(true, shapeDOM => {
          ...
          for (const { name, value } of Object.values(shapeDOM.attributes)) {
            if (!symbolAttributes.has(name)) {
              shapeDOM.removeAttribute(name);
            }
            // Hack to enable us to define a role in the symbol as it isn't registered as a dom attribute here
            if (name === 'aria-roledescription') {
              shapeDOM.setAttribute('role', value);
              shapeDOM.removeAttribute('aria-roledescription');
            }
          }
