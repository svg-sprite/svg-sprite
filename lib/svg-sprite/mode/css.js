...
SVGSpriteCss.prototype._init = function () {
  ...
  if (this.config.dimensions && this.config.dimensions !== true) {
    this.config.dimensions = /%s/g.test(
      (this.config.dimensions || '').split('%%').join('')
    )
  ...
  switch (typeof this.config.mixin) {
    ...
    case 'string': {
      this.config.mixin = this.config.mixin.trim().length
        ? this.config.mixin.trim()
        : null;
      break;
    }
  ...
  this._displaceable = [this.LAYOUT_VERTICAL, this.LAYOUT_HORIZONTAL].includes(
    this.config.layout
  );

  this._precision =
    Number(this.config.svg.precision) >= 0
      ? 10 ** Number(this.config.svg.precision)
      : null;
};
...
SVGSpriteCss.prototype.layout = function (files, cb) {
  ...
  files.sprite = this._buildSVG(
    config.xmlDeclaration || '',
    config.doctypeDeclaration || ''
  );
  this._spriter.verbose(
    'Created «%s» SVG sprite file («%s» mode)',
    this.key,
    this.mode
  );
...
SVGSpriteCss.prototype._layout = function () {
  ...
  for (const shape of this._spriter._shapes) {
    pseudoShapeMap[shape.base] =
      pseudoShapeMap[shape.base] || Boolean(shape.state);
  }

  // Layout the sprite
  this[
    this.config.layout === this.LAYOUT_PACKED
      ? '_layoutBinPacked'
      : '_layoutSimple'
  ](pseudoShapeMap);
  ...
  for (const [index, shape] of this.data.shapes.entries()) {
    ...
    if (this._displaceable || !shape.master) {
      ...
      if (this.config.layout === this.LAYOUT_VERTICAL) {
        ...
        shape.position.absolute.x = this._spriter._shapes[index]._round(
          (-x * (this.data.spriteWidth - shape.width.outer)) / 100
        );

        // Else: Determine the relative horizontal position
      } else {
        x = shape.position.absolute.x
          ? (100 * Math.abs(shape.position.absolute.x)) /
            (this.data.spriteWidth - shape.width.outer)
          : 0;
      }
      ...
      if (this.config.layout === this.LAYOUT_HORIZONTAL) {
        ...
        shape.position.absolute.y = this._spriter._shapes[index]._round(
          (-y * (this.data.spriteHeight - shape.height.outer)) / 100
        );

        // Else: Determine the relative vertical position
      } else {
        y = shape.position.absolute.y
          ? (100 * Math.abs(shape.position.absolute.y)) /
            (this.data.spriteHeight - shape.height.outer)
          : 0;
      }
      ...
      shape.position.relative = {
        x: this._round(x),
        y: this._round(y),
        xy: `${this._addUnit(this._round(x), '%')} ${this._addUnit(
          this._round(y),
          '%'
        )}`
      };
      ...
      if (svgX.length > 1) {
        x = shape.master
          ? shape.position.absolute.x - positionMap[shape.master].x
      ...
      if (svgY.length > 1) {
        y = shape.master
          ? shape.position.absolute.y - positionMap[shape.master].y
 ...
 */
SVGSpriteCss.prototype._addShapeToSimpleCssSprite = function (
  shape,
  needsRegular,
  index,
  position
...
SVGSpriteCss.prototype._layoutBinPacked = function (pseudoShapeMap) {
  ...
  for (const [index, shape] of this._spriter._shapes.entries()) {
    ...
    this.data.spriteWidth = Math.max(
      this.data.spriteWidth,
      Math.ceil(position.x + dimensions.width)
    );
    this.data.spriteHeight = Math.max(
      this.data.spriteHeight,
      Math.ceil(position.y + dimensions.height)
    );
 ...
 */
SVGSpriteCss.prototype._refineRootAttributes = function (
  shape,
  index,
  rootAttributes
) {
 ...
 */
SVGSpriteCss.prototype._addShapeToCSSSprite = function (
  shape,
  needsRegular,
  index,
  position,
        ...
        ? [
            {
              expression: format(
                this.config.prefix,
                shape.base + (shape.state ? `:${shape.state}` : '')
              ),
              raw: format(
                this.config.prefix,
                shape.base + (shape.state ? `:${shape.state}` : '')
            ...
            },
            {
              expression: format(
                this.config.prefix,
                `${shape.base}\\:${shape.state || 'regular'}`
              ),
              raw: format(
                this.config.prefix,
                `${shape.base}:${shape.state || 'regular'}`
  ...
  this.data.shapes[index] = {
    ...
    position: {
      absolute: {
        x: positionX,
        y: positionY,
        xy: `${this._addUnit(positionX, 'px')} ${this._addUnit(
          positionY,
          'px'
        )}`
      }
    ...
    dimensions: {
      ...
      extra: Boolean(
        isString(this.config.dimensions) && this.config.dimensions.length
      )
    }
  };
 ...
 */
SVGSpriteCss.prototype._buildSVG = function (
  xmlDeclaration,
  doctypeDeclaration
) {
  ...
  const _xmlDeclaration = this.declaration(
    this.config.svg.xmlDeclaration,
    xmlDeclaration
  );
  const _doctypeDeclaration = this.declaration(
    this.config.svg.doctypeDeclaration,
    doctypeDeclaration
  );

  const svg = new SVGSprite(
    _xmlDeclaration,
    _doctypeDeclaration,
    rootAttributes,
    true,
    this.config.svg.transform
  );
...
/**
 * Round a number considering the given decimal place precision for CSS
 * positioning values
 *
...
SVGSpriteCss.prototype._round = function (n) {
  return this._precision
    ? Math.round(n * this._precision) / this._precision
    : n;
};
