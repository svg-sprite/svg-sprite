...
module.exports = class SVGSpriteCssPacker {
  ...
  fit() {
    ...
    for (const { index, width, height } of this.blocks) {
      ...
      const { x, y } = node
        ? this._splitNode(node, width, height)
        : this._growNode(width, height);
  ...
  _findNode(root, width, height) {
    if (root.used) {
      return (
        this._findNode(root.right, width, height) ||
        this._findNode(root.down, width, height)
      );
    }
  ...
  _growNode(width, height) {
    ...
    const shouldGrowRight =
      canGrowRight && this.root.height >= this.root.width + width;
    const shouldGrowBottom =
      canGrowBottom && this.root.width >= this.root.height + height;
