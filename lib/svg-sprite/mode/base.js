...
function SVGSpriteBase(spriter, config, data, key) {
	...
	this.data						= _.merge(this.data, this._initData({
		...
		sprite						: path.relative(this.config.dest, this.config.sprite).split(path.sep).join('/')
	}));
...
SVGSpriteBase.prototype._addCacheBusting = function(svg) {
	...
	if (this.config.bust) {
		...
		this.data.sprite			= path.relative(this.config.dest, sprite).split(path.sep).join('/');
		if (this.config.example) {
